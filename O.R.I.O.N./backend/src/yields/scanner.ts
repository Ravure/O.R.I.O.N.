 ...
 * - Backup data from protocol subgraphs
...
import { getYieldHistoryStorage, YieldHistoryStorage } from '../storage/yieldHistory.js';
...
export class YieldScanner {
  ...
  private storage: YieldHistoryStorage | null = null;
  private persistResults: boolean = false;

  constructor(config: Partial<YieldScannerConfig> = {}, options?: { persist?: boolean; dbPath?: string }) {
    ...
    if (options?.persist) {
      ...
      try {
        this.storage = getYieldHistoryStorage(options.dbPath);
        console.log('[YieldScanner] Persistence enabled');
      } catch (error) {
        console.warn('[YieldScanner] Failed to initialize storage, persistence disabled:', error);
   ...
   */
  setPersistence(enabled: boolean, dbPath?: string): void {
    if (enabled && !this.storage) {
      try {
        this.storage = getYieldHistoryStorage(dbPath);
        this.persistResults = true;
        console.log('[YieldScanner] Persistence enabled');
      } catch (error) {
        console.warn('[YieldScanner] Failed to initialize storage:', error);
      }
    } else {
  ...
  /**
   * Get the storage instance (for direct queries)
   */
  getStorage(): YieldHistoryStorage | null {
    return this.storage;
  }
  ...
  async scanAllChains(forceRefresh = false): Promise<YieldScanResult> {
    ...
    if (this.persistResults && this.storage) {
      try {
        this.storage.storeScanResult(result, durationMs);
        console.log('[YieldScanner] Results persisted to database');
      } catch (error) {
        console.warn('[YieldScanner] Failed to persist results:', error);
      }
    }

    // Print summary
    this.printSummary(result, durationMs);
  ...
  private aggregateByChain(pools: YieldPool[]): ChainYields {
    ...
    for (const chainData of Object.values(yields)) {
      if (chainData.poolCount > 0) {
        chainData.avgApy = chainData.pools.reduce((sum: number, p: YieldPool) => sum + p.apy, 0) / chainData.poolCount;
      }
    }
  ...
  private calculateStats(
    ...
    return {
      ...
      avgApy: pools.length > 0
        ? pools.reduce((sum: number, p: YieldPool) => sum + p.apy, 0) / pools.length
        : 0,
